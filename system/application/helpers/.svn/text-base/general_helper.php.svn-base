...
function make_seo_url($value)
{
	$value=trim($value);
	$pattern='/[\,\s\-\/]/';
	$replace_value='_';
	$return_value=preg_replace($pattern,$replace_value,$value);
	return $return_value;
}
...
function current_date_time_stamp()
{
	...
	$site_info=$CI->Site_setting_model->get_site_info(1);
...
function calculate_time($target_date)
{

	$today=current_date_time_stamp();
	
	$difference =($target_date-$today);
	
	$day = floor($difference/60/60/24);
	$hour = floor($difference / 3600) - ($day * 24);
	$minute = floor($difference / 60) - ($day * 24 *60) - ($hour * 60);
	$second = floor($difference) - ($day* 24 *60*60) - ($hour * 60 * 60) - ($minute*60);
	...
	if($difference > 0 )
	{
		$return_value['output']=$hour.":".$minute.":".$second;	
		$return_value['flag']=1;
		
	}
	else
	{
		$return_value['output']='End';	
		$return_value['flag']=0;
	}
	return $return_value;
}
