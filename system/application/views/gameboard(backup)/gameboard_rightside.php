<div class="rightside">
            <div class="rightsideInner">
                <div class="content_wrap">
                    <div class="whiteboxrightside_topborder1"></div>
                    <div class="whiteboxrightside_bg">
                        <div class="whiteboxrightside_bgInner">
                        	<div class="content_10box">
                                <div class="text_center">
                                    <img src="<?=base_url()?>images/advertisement.jpg" width="250" height="200" alt="advertisement" />
                                </div>
                            </div>


                        </div>
                        <div class="content_wrap">
                        <div class="">
                            <div class="title_align">
                                <div class="greentitlebg_leftborder"></div>
                                <div class="greentitlebg_bg" style="width:230px;">
                                    <div class="bold font14 color_white">Wannaquiz help</div>
                                </div>
                                <div class="greentitlebg_rightborder"></div>

                                <div class="clear"></div>
                            </div>
                        </div>
                        <div class="">
                            <div class="whiteboxrightside_bgInner">
                            	<div class="help_links">
                                	<ul>
                                    	<li><a href="<?=base_url()?>page/show/faq" class="helpselected">Help files</a></li>
                                        <li><a href="<?=base_url()?>page/show/making_video_question">Make video question</a></li>
                                        <li><a href="<?=base_url()?>page/show/making_photo_question">Make photo question</a></li>
                                        <li><a href="<?=base_url()?>home/help_center/faq">FAQ</a></li>
                                        <li><a href="<?=base_url()?>home/help_center/advertise">Advertise</a></li>
                                        <li><a href="<?=base_url()?>page/show/register">Register</a></li>
                                        <li><a href="<?=base_url()?>page/show/login">Login</a></li>

                                    </ul>
                                    <div class="borderbottom_dotted"></div>
                                </div>
                            </div>
                        </div>
                        <div class=""></div>
                    </div>
                    </div>
                    <div class="whiteboxrightside_bottomborder"></div>

                </div>

                
            </div>
</div>