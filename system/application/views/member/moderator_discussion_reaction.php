...
{ 
	...
	if(job==true)
        ...
        {   j.post("<?=site_url('moderator_management/delete_discussion_comment/')?>", {comment_id:id}, function(data){
                    if(data=='success');
 ...
 function discussion_edit_comment(uid){ //alert('hii');
                ...
                if(comment == '')
               {
                ...
                {
                    j.post('<?=base_url()?>moderator_management/discussion_comment_edit', 
...
<div class="rounded_white_bg">
      <div class="pos">
        ...
        <div class="list_cont">
         <? foreach($discussion_reaction as $rows){
          ...
          <div class="list_slot clearfix reported_user_list" id="list_<?=$rows['id']?>">
            ...
            <div class="reported_comments_desc">
                 ...
                 <strong>Discussion:</strong> <a href="#"><?=$rows['discussion_title']?></a>
                 </div>
                 <div class="report_meta">Commented by <a href="<?=site_url($rows['username'])?>" class="reported_author"><?=$rows['username']?></a> | Date: <?=$rows['comment_date']?></div>
       <div style="display:none" id="editcomment_<?=$rows['id']?>"
        
            <label><strong class="font11 lh1_66" >Edit this Comment</strong></label>
            <textarea rows="20" cols="20" class="large_textarea" id="edit_comments_<?=$rows['id']?>"value="<?=$rows['comment']?>"><?=$rows['comment']?></textarea>
            <div class="clear"></div>
            <div class="clearfix"><span class="scalable_btn aqua">
              <input type="submit" value="Save edited disscussion" onclick="discussion_edit_comment('<?=$rows['id']?>')"/>
              </span></div>
          
       </div>
            
            </div>
            <div class="delete"> <a href="javascript:void(0)" onclick='return delete_comment("<?=$rows['id']?>")' class="delete_btn">Delete </a> 
                <a style="cursor:pointer"  class="delete_btn black_btn edit_comment" onclick="toggle('editcomment_<?=$rows['id']?>')">Edit Comment</a>
                </div>
          </div>
          <?php } ?>
        </div>
