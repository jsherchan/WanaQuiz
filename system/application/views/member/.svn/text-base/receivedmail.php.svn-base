...
$(document).ready(function()
{
	...
	var tt='';
	for (var i=0;i<document.receivedmsg.elements.length;i++)
	{
		var e=document.receivedmsg.elements[i];
	...
	if(tt=="")
		alert('Please check the message to delete');
            ...
            if(v){
		$.post('<?=base_url()?>member/deleteBulkMessages/received', {ids:tt} , function(data){			
...
function checkAll()
{
	for (var i=0;i<document.receivedmsg.elements.length;i++)
	{
		...
		if ((e.name != 'allbox') && (e.type=='checkbox'))
		{
			e.checked=document.receivedmsg.allbox.checked;
		}
	}
}
                    ...
                    <div class="content_wrap">
                        ...
                        <div class="whiteboxmidside_bg">
                            <div class="whiteboxrightside_bgInner">
                            	<form name="receivedmsg" action="" method="post">
                                	<div class="content_10box">
                                		<div class="bold">Received Message</div>
                                        <div class="content_10box">
                                            ...
                                            <div>
                                                ...
                                                <div class="padding_2bottom">
                                                    <div class="bg_lightblue">
                                                    	<div>
                                                        	<div class="msg_checkbox">
                                                                    <input type="checkbox" name="mailids[]" value="<?=$msg->id;?>"/>
                                                                </div>
                                                        	<div class="msg_from">
                                                                   
                                                                    <?php if($this->Member_model->get_sender_receiver($msg->id,0)=='') echo 'Admin'; else echo $this->Member_model->get_sender_receiver($msg->id,0);?>
                                                                </div>
                                                            <div class="msg_subject"><a href="<?=site_url('member/messageDetailReceive/'.$msg->id)?>"><?=$msg->subject?></a></div>
                                                            <div class="msg_date"><?=date('Y-m-d',$msg->created)?></div>
                                                           <? if($msg->read_flag==0){?> <div class="msg_new"><div class="newmsg_icon"><a href="<?=site_url('member/messageDetail/'.$msg->id)?>">New !</a></div></div><? }?>
