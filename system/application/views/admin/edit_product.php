...
function select_subcategory(id){
    ...
    jQuery.post("<?=base_url().ADMIN_PATH ?>/banner_management/get_category_questions", {cid:id}, function(data){
       if(data!=null && data!='' && data!=undefined){
           jQuery('#questions').html(data);
       }
       //alert(data);
    });
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
        <td colspan="2">
            ...
            <form name="frm_banner" method="post" enctype="multipart/form-data"  action="<?=site_url(ADMIN_PATH.'/product_management/update_product/')?>" onSubmit="return submitForm('');" >
                            ...
                            <table width="95%" border="0" cellspacing="1" cellpadding="4" >
                                <tr>
                                    ...
                                    <td width="75%" align="left" valign="middle" class="cat_block1">
                                        <input name="product_name" type="text" class="comment" value="<?php if($product_info) echo $product_info->product_name?>" size="50"></td>
                                </tr>
                                <tr>
                                    <td width="25%" align="right" valign="middle" class="cat_block1">Category<span class="style1">*</span></td>
                                    <td width="75%" align="left" valign="middle" class="cat_block1">
                                        <?php if($product_info)
                                        $category_name = $this->Category_model->get_category_by_id($product_info->category_id);
                                        //print_r($category);
                                        ?>
                                        <select name="category" onchange="get_subcategory()" id="category">
                                            <option value="<?php if($product_info) echo $category_name->id?>" /><?php if($product_info) echo $category_name->name?></option>
                                ...
                                <tr>
                                    ...
                                    <td width="75%" align="left" valign="middle" class="cat_block1">
                                       <div class="select_wrap" >
                                           <textarea id="product_description" name="product_description"><?php if($product_info) echo $product_info->product_description?></textarea>
                                        </div>
