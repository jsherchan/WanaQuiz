...
{ 
	...
	if(job==true)
        ...
        {   j.post("<?=site_url(ADMIN_PATH.'/moderator_activities/delete_discussion_comment')?>", {comment_id:id}, function(data){
                    if(data=='success');
 ...
 function discussion_edit_comment(uid){ //alert('hii');
                ...
                if(comment == '')
               {
                ...
                {
                    j.post('<?=site_url(ADMIN_PATH.'/moderator_activities/discussion_comment_edit')?>', 
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
    <tr>
        <td width="86%" height="40"> <span class="header"><a href="<?=site_url(ADMIN_PATH.'/home/')?>">ADMIN</a>
                >><a href="<?=site_url(ADMIN_PATH.'/moderator_activities/')?>"> Moderator Activities </a>>>Reported Forum Reactions</span></td>
...
<table width="100%" border="0" cellspacing="0" cellpadding="4" class="ttable v_top"">
    <tr>
        ...
        <? 
        $j=1;
        if(count($discussion_reaction)!=0){
    ...
    <tr id="list_<?=$rows['id']?>">
        ...
        <td align="left"><a href="<?=site_url(ADMIN_PATH.'/forum/forum_discussion_detail/'.$rows['disc_id']);?>"><?=$rows['discussion_title']?></a></td>
    ...
    <div ><span >
    <input type="submit" value="Save" id="submit_comment" onclick="discussion_edit_comment('<?=$rows['id']?>')"/>
    </span></div>

    </div>
        </td>
        <td align="left"><?=$rows['comment_date'];?></td>
      <td>
          <a href="javascript:void(0)" onclick='return delete_comment("<?=$rows['id']?>")' >Delete </a> |
                <a style="cursor:pointer"   onclick="toggle('editcomment_<?=$rows['id']?>')">Edit Comment</a>
                </td>
  </tr>
        <?php  $j++;}
        }
