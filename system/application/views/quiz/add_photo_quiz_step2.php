<? $user_quiz_budget = $this->Quiz_model->get_user_quiz_budget($this->session->userdata('wannaquiz_user_id'));?>
...
<script>
    ...
    $(document).ready(function () {
        ...
        //alert(cat_id);
        $('#product_'+cat_id).addClass('bg_skyblue').children('.bold').addClass('color_white');
    ...
    function form_validation(){        
        ...
        //var a = '< ?=$user_quiz_budget->budget_status?>';
        //if(a!=1 || a!='') { alert('here');return; document.forms.quizmaking1.submit(); }
      ...
      if(!custom_quiz_budget)
        ...
        var advertiser_budget = $('#advertiser_budget').val();
            if(!advertiser_budget) { document.forms.quizmaking1.submit(); return false; }
...
//        { $.prompt("Please Select Either 'From Remaining Credits' or 'PayPal' to Set Ad Budget"); $('#tip').show(); return false; }
        ...
        if(quiz_budget < user_credit)
            ...
            (
                'You will now have '+(user_credit-quiz_budget)+' Credits remaining in your Account' ,
                {
                    ...
                    {                        
                        if(v) { document.forms.quizmaking1.submit(); }
                    }
                }
            );
    ...
    function check_custom_budget(val){
        ...
        <div class="whiteboxrightside_bgInner">
            ...
            <div class="content_10box">
                <div class="padding_10leftright">
                    ...
                    <?php 
                     //  print_r($user_quiz_budget);
                    if($edit == 1) $action = site_url('quiz/addPhotoQuizStep3/1');
                    else
                    {
                        if($user_quiz_budget->budget_status == '1') $action = site_url('quiz/insert_quiz/0');
                        else $action = site_url('quiz/quiz_budget');

                                       if($edit == 1)
                    $action = site_url('quiz/addPhotoQuizStep3/1');
                    else{
                    ...
                    ?>
                    <form name="quizmaking1" action="<?=$action?>" method="post" id="quizmaking1">
                            <div style="width:630px;">
                            <input type="hidden" name="quiz_type" value="photo">
                                            ...
                                            <div class="content_10box">
                                                <div class="desc">
                                                    ...
                                                    if(count($product)>0) {
                                                        ...
                                                        if($edit!=0){
                                                            $product_id = $quiz_info->quiz_id;
                                                            $cat_status = $this->Category_model->get_category_by_id($quiz_info->category_id);
                                                            //print_r($cat_status);
                                                            if($cat_status->parent_id == '0')
                                                            $cat_id = $quiz_info->category_id;
                                                            else {
                                                                $cat_id = $cat_status->parent_id;
                                                                $sub_cat_id = $quiz_info->category_id;
                                                            }
                                                        }
                                                        else{
                                                            $product_id = $products->id;
                                                            $cat_id = $products->category_id;
                                                            $sub_cat_id = $products->sub_category_id;
                                                        }
                                                    ...
                                                    <div class="content_wrap product" onclick="select_category('<?=$products->id?>','<?=$products->category_id?>','<?=$products->sub_category_id?>')">
                                                        ...
                                                        <input type="hidden" id="category_id" value="<?=$cat_id?>">
                                                        <input type="hidden" id="sub_category_id" value="<?=$sub_cat_id?>">
                            ...
                            <?php if($edit !=0) {
                                $category_info = $this->Quiz_model->get_category_cpc($quiz_info->category_id);
                                ?>
                                ...
                                foreach($category as $categories) {
                                    ...
                                    if(count($subcategory)>0) {
                                        foreach($subcategory as $subcategories) {?>
                                            <option style="margin-left:20px; " value="<?php echo $subcategories->id; ?>"><?php echo $subcategories->name; ?></option>
                                            ...
                                            <!--<div class="quizmakingvideoform_right">
                                                <div class="padding_5bottom">
                                                    <div class="quizmakingform_radio">
                                                        ...
                                                        <label style="width:200px;"><?php if($edit!=0) echo $category_info->name; else$categories->name?></label>
                                                    <label>CPC <span class="color_green bold"><?php if($edit!=0) echo $category_info->cpc; else $categories->cpc?></span></label>
                                                    </div>
                                                    <div class="clear"></div>
                                                </div>
                                            </div>-->
                            ...
                            <?php if($user_quiz_budget->budget_status != '1' && $user_quiz_budget->budget_status == '') { ?>
                            <div class="content_wrap">
                                ...
                                <div class="content_10box">
                                    <div class="budgetform">
                                        <div class="input_clear">
                                            ...
                                            <span id="tip" style="display:none;">&nbsp;&nbsp;* change value to uncheck payment method</span>
                                            <select name="advertiser_budget" onchange="check_custom_budget(this.value)" id="advertiser_budget">
                                                <!--?php if($user_credits > 0) { ?>
                                                            <option value="rem_credits">From Remaining Credits ( <?=$user_credits?> ) </option-->
                                                <?php #} 
                                                       if($edit!=0) {$budget = $this->Quiz_model->get_quiz_budget($quiz_id);
                                                        ?>
                            ...
                            <div class="input_clear">
                                <div style="padding-left:180px;">
                                    ...
                                    <input type="button" class="searchbtn_bg" value="Continue" name="submit1" onclick="form_validation()" id="submit1" />
