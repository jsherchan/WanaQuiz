    ...
    function view($id='',$off='') {
      ...
      // filter content 
        ...
        $data['mem_info']=$this->Member_model->get_member($uid);
         $filter= $data['mem_info']->filter_adult;
        $data['filter']=$filter;
        ...
        $quiz_info= $this->Quiz_model->get_photo_quiz_detail($id,$filter);
  ...
  //echo $id;exit;
        ...
        $budget_status = $check_quiz_budget->budget_status;
         ...
         if($budget_status!='1'){
            ...
            $data['quiz_comments']=$this->Quiz_model->get_quiz_comments($qid,0,0);
            $result=$this->Quiz_model->get_quiz_comments($id,0,0);
            $config['base_url'] = base_url().'quiz/views/'.$id;
    ...
    function addVideoQuizStep2($action) {
        ...
        if($this->input->post('country_target',TRUE))
                {
                    foreach($this->input->post('country_target',TRUE) as $rows){
                    $country_target.=$rows.",";
                    }
                }
                else $country_target ='';
                ...
                {
                    foreach($this->input->post('state',TRUE) as $rows){
                    $state_target.=$rows.",";
                    }
                }
                else $state_target ='';
                ...
                {
                    foreach($this->input->post('city',TRUE) as $rows){
                    $city_target.=$rows.",";
                    }
                }
                else $city_target ='';
         ...
         $this->session->set_userdata(array(
            ...
            'quiz_question'=>$this->input->post('quiz_question',TRUE),
            'quiz_level'=>$this->input->post('quiz_level',TRUE),
            'quiz_question_type'=>$this->input->post('quiz_question_type',TRUE),
            'option1'=>$this->input->post('option1',TRUE),
            'option2'=>$this->input->post('option2',TRUE),
            'option3'=>$this->input->post('option3',TRUE),
            'right_answer'=>$this->input->post('right_answer',TRUE),
            'single_open_answer'=>$this->input->post('single_question',TRUE),
            'quiz_suitable_for'=>$this->input->post('quiz_suitable_for',TRUE),
            'quiz_comment'=>$this->input->post('quiz_comment',TRUE),
            'quiz_id' => $this->input->post('quiz_id',TRUE),
            'quiz_videos'=>$this->input->post('quiz_videos',TRUE),
            'video_answer'=>$this->input->post('video_answer',TRUE),
    ...
    function addVideoQuizStep3($action) {
        ...
        $this->session->set_userdata(
            ...
            'check_for_future_question'=>$this->input->post('check_for_future_question',TRUE),
            'advertiser_budget'=>$this->input->post('advertiser_budget',TRUE),
            'advertiser_custom_budget'=>$this->input->post('advertiser_custom_budget',TRUE),
            'budget_per_selection'=>$this->input->post('budget_per_selection',TRUE),
            'budget_for'=>$this->input->post('budget_for',TRUE)
        ));
...
*/
        ...
        if($action == 1) {
         ...
         $user = $this->Quiz_model->get_user_quiz_budget($this->session->userdata('wannaquiz_user_id'));
        //$user = $this->Member_model->get_member($this->session->userdata('wannaquiz_user_id'));
        $data['user_credits'] = $user->total_budget;
        $data['product'] = $this->Quiz_model->get_products();
        $data['user_type'] = $user->user_type;
        $data['quiz_info'] = $this->Quiz_model->get_quiz_info_by_qid($data['quiz_id']);
        ...
        // echo($this->input->post('country_target'));
         if($this->input->post('country_target',TRUE))
           ...
           $this->session->set_userdata(
            ...
            'quiz_level' =>$this->input->post('quiz_level',TRUE),
            'quiz_question'=>$this->input->post('quiz_question',TRUE),
            'quiz_type'=>$this->input->post('quiz_question_type',TRUE),
            'option1'=>$this->input->post('option1',TRUE),
            'option2'=>$this->input->post('option2',TRUE),
            'option3'=>$this->input->post('option3',TRUE),
            'right_answer'=>$this->input->post('right_answer',TRUE),
            ...
            'quiz_suitable_for'=>$this->input->post('quiz_suitable_for',TRUE),
            'quiz_comment'=>$this->input->post('quiz_comment',TRUE),
            'quiz_long_answer'=>$this->input->post('quiz_long_answer',TRUE),
            'image_name'=> $this->input->post('ques_image',TRUE),//$new_image_file_name,
            'answer_image'=>$this->input->post('ans_image',TRUE),
    ...
    function addPhotoQuizStep3($action) {
        ...
        $custom_budget = $this->input->post('custom_quiz_budget',TRUE);
        if($custom_budget!='')
        $advertiser_budget = $custom_budget;
        else
        $advertiser_budget = $this->input->post('advertiser_budget',TRUE);

        $custom_per_selection = $this->input->post('custom_per_selection',TRUE);
    ...
    function insert_quiz($action){        
       ...
       // echo $this->input->post('category'); exit;
        $this->session->set_userdata( array('category_id' =>$this->input->post('category',TRUE)));
      ...
      // echo "quiz".$member_cpc;
      $cat_info = $this->Category_model->get_parent_id($this->input->post('category',TRUE));
    ...
    function payment() {
        ...
        $user_credits = $this->input->post('user_credits',TRUE);
        //echo $user_credits.'/'.$advertiser_budget; exit;

        if($this->input->post('user_credits',TRUE) > '0' && $this->input->post('user_credits',TRUE) > $advertiser_budget){
            $this->db->where('user_id',$this->session->userdata('wannaquiz_user_id'));
            $this->db->update('tbl_members',array('user_credits'=>($this->input->post('user_credits',TRUE)-$advertiser_budget)));

            $this->db->where('quiz_id',$quiz_id);
            $this->db->update('tbl_quizes',array('status'=>'1'));

            if ($ad_space!=''){
            $this->Quiz_model->editQuizBudget($this->input->post('quiz_id',TRUE));
            
        }
            
            $data['main']= "quiz/add_photo_quiz_success_msg";
        }
        else if($this->input->post('user_credits',TRUE) > '0' && $this->input->post('user_credits',TRUE) < $custom_budget){
            $this->db->where('user_id',$this->session->userdata('wannaquiz_user_id'));
            ...
            $this->db->where('user_id',$this->session->userdata('wannaquiz_user_id'));
            $this->db->update('tbl_members',array('user_credits'=>$this->input->post('user_credits',TRUE)+($coupon_amount - $advertiser_budget)));
    ...
    function quiz_budget(){
        ...
        $quiz_type=$this->input->post('quiz_type',TRUE);
        $quiz_id = $this->input->post('quiz_id',TRUE);
        $remaining_budget = $this->input->post('remaining_budget',TRUE);

        $custom_budget = $this->input->post('custom_quiz_budget',TRUE);
        $custom_per_selection = $this->input->post('custom_per_selection',TRUE);
        $pre_budget = $this->input->post('pre_budget',TRUE);
        $payment = $this->input->post('radio',TRUE);        
      
        if($custom_budget!='') $advertiser_budget = $custom_budget;
        else if($this->input->post('advertiser_budget',TRUE)=='rem_credits' || $this->input->post('advertiser_budget',TRUE)==0) $advertiser_budget = 0;
        else $advertiser_budget = $this->input->post('advertiser_budget',TRUE);

        if($custom_per_selection!='') $budget_per_selection = $custom_per_selection;
        else $budget_per_selection = $this->input->post('budget_per_selection',TRUE);
        ...
        $coupon_amount = 0;
        $coupon_code = trim($this->input->post('coupon',TRUE));
...
#exit;            
    ...
    #echo '<pre>' . print_r($coupon_info) . '</pre>';exit;
         ...
         else if( ($this->input->post('advertiser_budget',TRUE)=='rem_credits' || $payment=='user_credits') && $payment!='paypal')
         {
            ...
            $datas = array('user_id'=>$this->session->userdata('wannaquiz_user_id'),
                            'budget_per_selection'=>$budget_per_selection,
                            'per_selection'=>$this->input->post('budget_for',TRUE),
...
//        }
    ...
    function quizCommit() {
        ...
        if($data) {
            ...
            $qid = $this->input->post('quiz_id',TRUE);
...
//<a href='".ADMIN_PATH('comment_spam')."'>
 ...
 function debug(){
  print_r($this->session->userdata('debug'));
      echo  $this->session->userdata('debug');
 }
 function searchByCategory($a="",$category_id="",$sub_id="",$q_level="")
 {
     $uid=$this->session->userdata('wannaquiz_user_id');
     ...
     if($a==1){
       // $this->session->set_userdata('debug',$category_id."/".$sub_id."/".$q_level);
         $cat_id=$category_id;
         $subcat_id=$sub_id;
         $quiz_level=$q_level;
              }
...
//           echo "special".$this->session->userdata('special_quiz_next');
           ...
           //  echo "regular";
            $this->session->set_userdata('limit',$this->session->userdata('limit')+1);
            if($this->session->userdata('game_mode')=='' || $this->session->userdata('game_mode')=='single')
            $result = $this->Quiz_model->search_by_category($quiz_level,$cat_id,$subcat_id,'0',$filter);
            else  $result=$this->Quiz_model->getRandomQuizForMultiplayer($quiz_level,$cat_id,$subcat_id,'0',$filter);
           ...
           // echo "sponosr";
            $this->session->set_userdata('limit',$this->session->userdata('limit')+1);
            if($this->session->userdata('game_mode')=='' || $this->session->userdata('game_mode')=='single')
            $result = $this->Quiz_model->search_by_category($quiz_level,$cat_id,$subcat_id,'1',$filter);
            else $result=$this->Quiz_model->getRandomQuizForMultiplayer($quiz_level,$cat_id,$subcat_id,'1',$filter);
            ...
            $this->session->set_userdata('next_quiz','special');
             ...
             $this->session->set_userdata('limit',$this->session->userdata('limit')+1);
              if($this->session->userdata('game_mode')=='' || $this->session->userdata('game_mode')=='single')
              $result = $this->Quiz_model->search_by_category($quiz_level,$cat_id,$subcat_id,'2',$filter);
              else $result=$this->Quiz_model->getRandomQuizForMultiplayer($quiz_level,$cat_id,$subcat_id,'2',$filter);
     ...
     //echo "count".$this->session->userdata('count')."quiz id:";
      ...
      if($quiz_id=$result->quiz_id=='' && $this->session->userdata('limit')<3){
                // echo $this->session->userdata('limit');
                 $this->searchByCategory(1,$cat_id,$subcat_id,$quiz_level);
              }
                 else if($quiz_id=$result->quiz_id=='' && $this->session->userdata('limit')>=3 ) {
                    $user_tip=rand(3);
                   $result=$this->Quiz_model->getRandomQuiz($quiz_level,$user_tip,$filter);
                     echo $quiz_id=$result->quiz_id;
                       }
             else echo $quiz_id=$result->quiz_id;
    ...
    function categoryDetail($user_type,$cname) {
        ...
        $result=$this->Quiz_model->get_questions_by_category($cid,$user_type,$filter,0,0);
        $config['total_rows']=  count($result);
        $config['per_page'] = '15';
        ...
        $data['category_questions'] = $this->Quiz_model->get_questions_by_category($cid,$user_type,$filter,$config['per_page'],$offset);
    ...
    function getAnswer() {
        ...
        $sql="SELECT * from tbl_quizes q,tbl_quiz_options qo,tbl_quiz_videos qv where q.quiz_id=qo.quiz_id and q.quiz_id=qv.quiz_id AND q.status='1' and q.quiz_id=?";
    ...
    function playVideoAnswer() {
        $this->load->model('Award_model');
        $option_id=$this->input->post('option',TRUE);
        $quiz_id=$this->input->post('quiz_id',TRUE);
...
# here
            if($quiz_info->user_type!=0 && $advertise_id!='') 
            {
                ...
                $ad_content ='<div class="content_wrap" style="float:left;width:480px;margin-left:0px;text-align:left;">
    ...
    function showAnswerResultMulti() {
        ...
        if(!$this->session->userdata('player_id')) {
            ...
            if($this->input->post('flag',TRUE)!='play_further') {
...
//        $video_image = explode(".",$data->quiz_videos);
    ...
    function trackClick() {
        ...
        $ads_type = $this->input->post('ads_type',TRUE);
    ...
    function showMultiScore() {
        ...
        $type=$this->input->post('type',TRUE);
        $quiz_id=$this->input->post('quiz_id',TRUE);
        $previous_player = $this->input->post('previous_player',TRUE);
...
Note: multiplayer scores are saved and viewable here, not displayed on your profile page.
    ...
    function selectPlayers() {
        $this->load->model('Member_model');
        $play_mode=$this->input->post('type',TRUE);
...
Just click the checkboxes and replace \'player...\' with any name you want.
    ...
    function saveSettings() {
        ...
        $member_names=$this->input->post('member_names',TRUE);
    ...
    function deleteFavourites() {
        $quizid= $this->input->post('quiz_id',TRUE);
...
//        }
    ...
    function report_quiz() {
        ...
        //echo $reporter_id = $this->input->post('reporter_id',TRUE);
    ...
    function set_selected_category() {
        ...
        $class = $this->input->post('class_color',TRUE);
    ...
    function set_playlist_session() {
        ...
        $input_data = $this->input->post('playlist',TRUE);
        $user_id = $this->input->post('user_id',TRUE);
    ...
    function playlist_quiz_click_ses() {
        $user_id = $this->input->post('user_id',TRUE);
        $quiz_id = $this->input->post('quiz_id',TRUE);
        $playlist_id = $this->input->post('playlist',TRUE);
    ...
    function testVideoLength(){
       ...
       $question_file_name = $this->input->post('video_question',TRUE);
      // $question_file_name="test_sync2.wmv";
        $answer_file_name = $this->input->post('video_answer',TRUE);
        ...
        //echo "Question length:".$question_video_length;
        if($question_video_length > '00:00:20.00')
        echo "%question_error";
        else echo "%question_success";
        echo "%";
      // echo "Answer Lenth: ".$answer_video_length;
   ...
   function get_states()
     {
       $countriesArr = array(); 
       $country_code=$this->input->post('term',TRUE);
     
       $countrycodeArr = explode("|||", $country_code);
      
       $states =$this->Country_management_model->get_state($countrycodeArr);
