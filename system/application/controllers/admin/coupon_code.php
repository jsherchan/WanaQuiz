...
class Coupon_code extends Front_controller {
	...
	function edit_coupon($coupon_id)
	{
		$data['title']="Auction:Coupon Management";
 		$data['main']='admin/coupons';
		$data['coupon_info']=$this->Coupon_code_model->get_coupon_info_by_id($coupon_id);
		$data['view_coupons_layout']="no";
	...
	function view_coupons($sort_field,$sort_order)
	{
		...
		$config['base_url'] = site_url(ADMIN_PATH.'/coupon_code/view_coupons/'.$sort_field.'/'.$sort_order.'/');
		$coupon_info=$this->Coupon_code_model->get_all_coupons_info($sort_field,$sort_order,0,0);
		...
		$data['coupon_info']=$this->Coupon_code_model->get_all_coupons_info($sort_field,$sort_order,$config['per_page'],$offset);
	...
	function update()
	{
		$this->Coupon_code_model->edit();
		$this->session->set_flashdata('message','Coupon edited successfully.');
		redirect(ADMIN_PATH.'/coupon_code/');
	}
	
	function insert()
	{
		$this->Coupon_code_model->insert();
