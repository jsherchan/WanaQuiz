...
*/
    ...
    function editPhotoQuizAdvertiser() {
        ...
        $data=array(
            ...
            'images2'=>$this->session->userdata('answer_image')
        );
...
if($user_type != 0)
    ...
    function get_questions_by_category($cid,$user_type,$filter,$num,$offset) {
        $id = $this->session->userdata('wannaquiz_user_id');
        ...
        $sql="
           ...
           if($filter=='1' OR $filter=='')$sql.=" AND q.quiz_suitable_for='1'";
        ...
        $sql .="
            ...
            AND (q.state_target='' or ( q.state_target!='' AND q.state_target like '%".strtolower($_SESSION['state_target'])."%')) 
            AND (q.city_target='' or ( q.city_target!='' AND q.city_target like '%".strtolower($_SESSION['city_target'])."%'))
            and q.user_type='$user_type' AND q.status='1'";
          if($filter=='1' OR $filter=='')$sql.=" AND q.quiz_suitable_for='1'";
          $sql.=") $order_by $limit";
    ...
    function get_photo_quiz_detail($id,$filter="") {
        $sql = "select * from tbl_quizes where quiz_id =?";
         if($filter=='1' OR $filter=='') $sql.="and quiz_suitable_for='1'";
        $query = $this->db->query($sql, array($id));
        return $query->row();
    ...
    function get_quiz_by_user_category($uid,$id,$filter="") { //echo $uid.'/'.$cid.'/**';
  //        if($sid=='')
//        $array = $cid;
//        else{
//            $array = array($sid,$cid);
          ...
          if($filter=='1' OR $filter=='')$sql.="AND q.quiz_suitable_for='1'";
              $sql.=")
                ...
                (Select q.quiz_id,q.quiz_question,q.quiz_type,q.posted_date,qv.video_answer,qv.quiz_videos,m.username, qb.budget_status FROM
                    ...
                    q.user_id=m.user_id and q.user_id=qb.user_id and q.quiz_id=qv.quiz_id and q.user_id=? and q.category_id in($array) and q.user_type='advertiser' and qb.budget_status='1' and q.status='1'";
                   if($filter=='1' OR $filter=='')$sql.="AND q.quiz_suitable_for='1'";
                      $sql.=")) a order by a.posted_date desc";
        ...
        if(!$category) $category = '1';
         if(!$search_item) $sitem="q.quiz_question like '% %'";
          ...
          $sql="(Select q.*,qm.images,0,m.username,0,0
                ...
                and (q.city_target='' or ( q.city_target!='' and q.city_target like '%".strtolower($_SESSION['city_target'])."%')) 
                ";
        if($f=='1' OR $f=='') $sql .= "and q.quiz_suitable_for='1'";        
...
#exit($sql);
    ...
    function search_by_category($level,$category_id,$subcategory,$user_type='',$filter='') {
       ...
       // echo $category_id;
         $sql2 =  "select m.user_id from tbl_members m where user_type='$user_type'";
            ...
            $sql1 =  "Select c.id FROM tbl_categories c where c.parent_id='$category_id'";
             ...
             $value = implode(",",$a);
             if($value != ''){
           if($user_type!=0) $sql="select a.quiz_id from (select quiz_id,user_id"; 
                 else $sql= "SELECT quiz_id";
                     $sql.=" FROM  tbl_quizes 
            ...
            if($user_id) $sql.=" AND user_id NOT IN ($user_id)"; 
                $sql.=" order by rand()";
         if($user_type!=0) $sql.=")a join tbl_quiz_budgets b on a.user_id=b.user_id where b.budget_status='1'";
    $sql.= " Limit 1";
             }else{
                       if($user_type!=0) $sql="select a.quiz_id from (select quiz_id,user_id"; 
                                  else $sql= "SELECT quiz_id";
                            ...
                            ";
                     if($filter=='1' OR $filter=='')$sql.="AND quiz_suitable_for='1'";
                    if($user_id) $sql .= " AND a.quiz_id NOT IN
                            (SELECT QUIZ_ID FROM tbl_quiz_answers WHERE user_id = '$user_id')";
                    $sql .= " AND user_id IN ($user_idss)"; 
                    if($user_id) $sql.=" AND user_id NOT IN ($user_id) LIMIT 1";
            ...
            $sql.= " FROM tbl_quizes 
                    ...
                    AND quiz_level='$level' AND status='1'";
             if($filter=='1' OR $filter=='')$sql.="AND quiz_suitable_for='1'";
            if($user_id) $sql .= " AND quiz_id NOT IN
...
//    function get_playlist_by_id($id,$user_id) {
//        $sql="select p.*, q.* from tbl_playlist p join tbl_quiz_playlist q on p.id=q.playlist_id where p.user_id=? and q.playlist_id=? order by q.playlist_id";
...
//    }
    ...
    function get_quizes_from_playlist($user_id,$playlist_id,$filter="",$num,$offset) {
        if($num==0 && $offset==0)
            $limit='';
        else
            $limit=" LIMIT $offset,$num";
    ...
    function set_user_position($user_id) { //echo $user_id;
       ...
       // print_r($total_hard_points);
       // print_r($total_avg_points);
    ...
    function getRandomQuiz($level,$user_type="",$filter="") {
            $sql2 =  "select m.user_id from tbl_members m where user_type='$user_type'";
             $query = $this->db->query($sql2);
           //  echo $this->db->last_query();
             $result = $query->result();
        ...
        $ip_address = $_SERVER['REMOTE_ADDR'];
         if($level!=""){
            if($user_type!=0) $sql="select a.* from (select *"; 
                 else $sql= "SELECT *";      
                $sql.=" from tbl_quizes  where quiz_level='$level' and quiz_id NOT IN(SELECT mp.quiz_id from tbl_multiplayer_point mp where mp.ip_address = '$ip_address') AND user_id IN ($user_idss)"; 
                if($user_id)$sql.=" AND user_id NOT IN ($user_id)";
                if($filter=='1' OR $filter=='')$sql.="AND quiz_suitable_for='1'";
                $sql.=" order by rand()";
                  if($user_type!=0) $sql.=")a join tbl_quiz_budgets b on a.user_id=b.user_id where b.budget_status='1'";
       ...
       if($user_type!=0) $sql="select a.* from (select *"; 
               ...
               $sql.=" from tbl_quizes where quiz_id NOT IN(SELECT mp.quiz_id from tbl_multiplayer_point mp where mp.ip_address = '$ip_address') AND user_id IN ($user_idss)";
               if($user_id) $sql.=" AND user_id NOT IN ($user_id)";
               if($filter=='1' OR $filter=='')$sql.="AND quiz_suitable_for='1'";
               $sql.=" order by rand()";
               if($user_type!=0) $sql.=")a join tbl_quiz_budgets b on a.user_id=b.user_id where b.budget_status='1'";
  ...
  //  echo $this->db->last_query();
    ...
    function getRandomQuizForMultiplayer($level,$category_id,$subcategory,$user_type,$filter) {
...
//                    $this->Award_model->insertPerfectScore($user_id,'10H');
    ...
    function get_member_level($user_id){
        $sql = "select * from tbl_member_levels ml, tbl_level l where ml.level_id=l.level and ml.user_id =?";
        $query = $this->db->query($sql,array($user_id));
        if($query->num_rows()>0)
        return $query->row();
        else return null;
    ...
    function get_member_previous_level($user_id,$current_level){
        $sql = "select * from tbl_member_levels ml, tbl_level l where l.level = '$current_level' and ml.user_id = '$user_id'";
        $query = $this->db->query($sql);
        if($query->num_rows()>0)
        return $query->row();
        else return null;
...
//    }
    ...
    function update_coupon_code($code){
        ...
        $numbers_in_use = $coupon_info->numbers_in_use;
        $this->db->where('coupon_code',$code);
        $this->db->update('tbl_coupons', array('numbers_in_use'=>$numbers_in_use+1));
    }

    function insert_quiz_budget_from_paypal($gross_amount,$budget_per_selection,$user_id){
       
        $budget_info = $this->get_user_quiz_budget($user_id);        
        $total_bgt=$budget_info->total_budget;
        if($budget_info) $pre_budget = $budget_info->remaining_budget;
        else $pre_budget = 0;
         if($pre_budget > $budget_per_selection)
        {
            $s = '+';
            $total_budget = ($pre_budget - $budget_per_selection);
           
        }
        else if($pre_budget < $gross_amount)
        {
            $s = '-';
            $total_budget = ($budget_per_selection - $pre_budget);
        }
        else
        {
            $s = '+';
