...
class Forum_model extends Model {
    ...
    function get_sub_cat($id) {
        $qry = "select * from tbl_forum_category where parent_id = ?";
    ...
    function add_forum($user_id='') {
        ...
        $sticky_position = $this->input->post('sticky_postion');
        if ($sticky_position == '')
            $data['sticky_position'] = '100';
        else
            $data['sticky_position'] = $sticky_position;
...
#$this->insert_log($log_id);
    ...
    function get_all_discussion_by_category($cat_id, $num, $limit) {
        $sql = "select m.username, d.* from 
                    tbl_discussion d 
                       join tbl_members m on m.user_id=d.user_id where d.cat_id=?  and d.flag='1' order by d.sticky_position ASC limit " . $limit . "," . $num;
        $result = $this->db->query($sql, array($cat_id));
    ...
    function get_all_discussion_by_sub_category($sub_id, $num, $limit) {
        $sql = "select m.username, d.* from 
                    tbl_discussion d 
                       join tbl_members m on m.user_id=d.user_id where d.sub_cat_id=?  and d.flag='1' order by d.sticky_position ASC limit " . $limit . "," . $num;
        $res = $this->db->query($sql, array($sub_id));
        return $res->result();
    }
    ...
    function get_thread_count($id) {
        $sql = "SELECT count(disc_id) as post from tbl_discussion d
            join tbl_forum_categories c on d.cat_id=c.id where d.sub_cat_id=?";
    ...
    function get_last_post($id) {
        $sql = "SELECT m.username, d.disc_id, d.discussion_title, d.created from tbl_members m
                ...
                on m.user_id=d.user_id where  sub_cat_id=? order by d.created desc limit 1";
        $res = $this->db->query($sql, array($id));
        return $res;
    }
    ...
    function get_last_comment($id) {
        $sql = "SELECT m.username, d.discussion_title, e.comment_date from tbl_members m
                ...
                on m.user_id=d.user_id
                 join tbl_discussion_comment e on d.disc_id=e.disc_id where d.disc_id=?  order by e.comment_date desc limit 1 ";
        $res = $this->db->query($sql, array($id));
        return $res;
    }
    ...
    function forum_detail($disc_id) {
        $qry = "select * from tbl_discussion where disc_id='?'";
    ...
    function get_category_id($cat_id) {
        $query = "select name from tbl_forum_categories where id=?";
        $res = $this->db->query($query, array($cat_id));
        return $res->row_array();
    }

    function get_total_reply($id) {
        $sql = "SELECT count(c.disc_id) as total FROM tbl_discussion_comment c
            join tbl_discussion d on c.disc_id=d.disc_id where c.disc_id=?";
        $res = $this->db->query($sql, array($id));
        return $res->result_array();
    }

    function count_discussion($id) {
        $sql = " select count(cat_id) as catagory from tbl_discussion where cat_id=?";
        $res = $this->db->query($sql, array($id));
        return $res->row();
    }
    ...
    function get_comment_by_id($id, $num, $lim) {
        $query = "SELECT m.user_id, m.username, m.moderator, d.profile_picture, c.discussion_title, c.content, md.id, md.user_id, md.comment, md.comment_date
                ...
                where c.disc_id=? and md.status='1'group by md.id  limit " . $lim . "," . $num;
    ...
    function get_level($id) {
        $sql = "select a.level_name, m.current_points  from tbl_level a
            join tbl_member_levels m on a.id=m.level_id where m.user_id=?";
        $res = $this->db->query($sql, array($id));
        return $res->result_array();
    }
    ...
    function get_comment_by_forum_id($disc_id) {
        $qry = "select * from tbl_disc_comments where disc_id=? and parent_id = 0 order by added_date desc";
        $res = $this->db->query($qry, array($disc_id));
        return $res;
    }
    ...
    function get_child_comment($comment_id) {
        //   echo "ksjdf";
        $qry = "select * from tbl_disc_comments where parent_id =?";
        return $res = $this->db->query($qry, array($comment_id));
    }

    function get_num_comments($id) {
        $qry = "Select * from tbl_disc_comments where disc_id=?";
        $res = $this->db->query($qry, array($id));
        echo $res->num_rows();
    }

    function delete_comment($id) {
        $qry = "delete  from tbl_disc_comments where comment_id =? ";
    ...
    function get_similar_disc($disc_cat_id) {
        $qry = "select * from tbl_discussion where cat_id =?  limit 0,4";
        $res = $this->db->query($qry, array($disc_cat_id));
        return $res;
    }
...
limit 0,4";
    ...
    function get_catcount_forum($cat) {
        $qry = "select * from tbl_discussion where cat_id=?";
        $res = $this->db->query($qry, array($cat));
        return $res->num_rows();
    }

    function get_subcount_forum($cat) {
        $qry = "select * from tbl_discussion where sub_cat_id=?";
        $res = $this->db->query($qry, array($cat));
        return $res->num_rows();
    }
    ...
    function get_sub_forum($cat, $offset) {
        ...
        $qry = "select * from tbl_discussion where sub_cat_id=? order by created  DESC  limit $offset,5";
        $res = $this->db->query($qry, array($cat));
        $data = $res;
        $this->db->last_query();
        return $data;
    ...
    function get_cat_forum($cat, $offset) {
        ...
        $qry = "select * from tbl_discussion where cat_id=? order by created  DESC  limit $offset,5";
        $res = $this->db->query($qry, array($cat));
        $data = $res;
        $this->db->last_query();
        return $data;
    ...
    function countforum() {
        $qry = "select * from tbl_discussion where tags like '%?%'";
        $res = $this->db->query($qry, array($this->uri->segment(3)));
        return $res->num_rows;
    }

    function get_frontpage_forumbytag($offset) {

        $qry = "select * from tbl_discussion where tags like '%?%' order by created  DESC  limit $offset,5";
        $res = $this->db->query($qry, array($this->uri->segment(3)));
        $data = $res;
        $this->db->last_query();
        return $data;
...
WHERE disc_id =$disc_id";
    ...
    function count_rating($disc_id) {
        $qry = "SELECT count(no_rating) as count from tbl_forum_rating where disc_id =?";
        $res = $this->db->query($qry, array($disc_id));
        $data = $res->row();
        echo $data->count;
    }
    ...
    function check_rating($disc_id) {
        $user_id = $this->session->userdata('user_id');
        $qry = "select * from tbl_forum_rating where disc_id =?  and user_id =? ";
        $res = $this->db->query($qry, array($disc_id, $user_id));
        $num = $res->num_rows();
        return $num;
    }
...
WHERE w.user_id =?";
    ...
    function get_forum_by_id($id) {
        $sql = "SELECT m.username, m.joined_date, g.moderator, p.profile_picture, d.* from tbl_members as m
          ...
          join tbl_discussion d
           on m.user_id=d.user_id where d.disc_id=? and d.flag='1'";
    ...
    function get_forum_by_ids($id) {
        $sql = "SELECT m.username, m.joined_date, g.moderator, p.profile_picture, d.* from tbl_members as m
            join tbl_members g on g.user_id=m.user_id
           join tbl_member_profile p on m.user_id=p.member_id
          join tbl_discussion d
           on m.user_id=d.user_id where d.disc_id=?";
    ...
    function get_user($user) {
        $query = "select user_id,username from tbl_members where user_id=?";
    ...
    function get_category($cat_id) {
        $query = "select * from tbl_forum_category where id =? ";
        $res = $this->db->query($query,array($cat_id));
        $data = $res->result();
        foreach ($data as $val) {
            return $val;
    ...
    function get_blog_title($id) {
        $qry = "select * from tbl_blog where blog_id = '?'";
        $res = $this->db->query($qry,array($id));
        $data = $res->result_array();
        foreach ($data as $d) {
            echo $d['blog_title'];
...
WHERE mf.user_id = ? AND mf.friend_id NOT IN(SELECT member_id FROM tbl_disc_tipped WHERE user_id = ?)";
    ...
    function search_forum($f) {
        ...
        $qry = "SELECT * 
       FROM `tbl_discussion`
        WHERE `discussion_title` LIKE '%?%'";
        $res = $this->db->query($qry,array($value));
...
//        echo $this->db->last_query();
    ...
    function add_view222($rec_id) {
        ...
        $qry = "insert into tbl_forum_view set forum_id ='?', user_id='?'";
        $this->db->query($qry,array($rec_id,$user_id));
    ...
    function forum_view($recipe_id) {
        ...
        $qry = "SELECT count( v.forum_id ) forumcount, v.forum_id
                            ...
                            WHERE d.sub_cat_id=? and v.user_id
                            IN (
                            SELECT user_id
                            FROM `tbl_forum_view`
                            WHERE forum_id =?
                            ...
                            LIMIT 0 , 3";
        $res = $this->db->query($qry,array($row->id,$recipe_id,$recipe_id));
        return($res);
    }
    ...
    function count_number_of_post() {
        $sql = "select count(m.user_id) as totalPost , m.user_id from tbl_discussion d left join tbl_members m on (d.user_id= m.user_id)group by m.user_id";
        $res = $this->db->query($sql);
        return $res->result_array();
    }
